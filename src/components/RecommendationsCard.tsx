      ...
      <div className="space-y-3 flex items-center justify-between gap-2">
        {recommendations.slice(0, 4).map((rec, index) => (
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${rec.colorClass} opacity-60 group-hover:opacity-80 transition-all`}
            ></div>

            {/* icon */}
            <div
              className={`relative z-10 text-emerald-500 bg-white/70 p-3 rounded-xl shadow-inner flex items-center justify-center mb-2 transition-transform group-hover:scale-110`}
            >
